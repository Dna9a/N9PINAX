...
def _probe_tcp_syn(ip: str, port: int = 80) -> dict | None:
    ...
    try:
        ...
        _log.debug("scapy unavailable — TCP SYN probe skipped")
        return None
    ...
    try:
        ...
        _log.warning("TCP SYN probe requires CAP_NET_RAW / root")
        return None
    except Exception as e:
        _log.debug("TCP SYN probe %s:%d failed: %s", ip, port, e)
        return None
...
def _probe_icmp(ip: str) -> dict | None:
    ...
    try:
        ...
        _log.warning("ICMP probe requires CAP_NET_RAW / root")
        return {"ttl": None, "icmp_response": False}
    except Exception as e:
