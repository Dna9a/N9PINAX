...
class AlertSeverity(str, Enum):
    """Severity level of an alert generated by the SIEM engine."""
...
def _normalize_mac(raw: str) -> str:
    ...
    if not isinstance(raw, str):
        raise TypeError("L'adresse MAC doit être une chaîne de caractères.")
...
class Port(BaseModel):
    ...
    banner: Optional[str] = Field(
        None,
        description="Bannière brute renvoyée par le service (automatiquement tronquée à 512 chars).",
        examples=["Apache/2.4.54 (Ubuntu)"],
    )

    @model_validator(mode="before")
    @classmethod
    def truncate_banner(cls, data: dict) -> dict:
        """Tronque la bannière si elle dépasse la limite avant validation."""
        if isinstance(data, dict) and data.get("banner"):
            if isinstance(data["banner"], str) and len(data["banner"]) > 512:
                data["banner"] = data["banner"][:512]
        return data
    ...
    @classmethod
    def sanitize_banner(cls, v: Optional[str]) -> Optional[str]:
        """Retire les caractères non imprimables de la bannière."""
        if v is None:
            return None
        return "".join(c for c in v if c.isprintable()).strip() or None
...
class Device(BaseModel):
    ...
    latency_ms: Optional[float] = Field(
        None,
        ge=0.0,
        description="Round-trip latency in milliseconds (from ICMP/TCP probe).",
    )
    ...
    def risk_label(self, alerts: Optional[list["Alert"]] = None) -> str:
        """
        Risk level for this device — ``"low"`` | ``"medium"`` | ``"high"``.

        Derived from the highest severity of any alert linked to this device
        (by IP or MAC); falls back to a port-exposure heuristic when no alerts
        apply. See :func:`compute_device_risk`.
        """
        return compute_device_risk(self, alerts)


def _device_quality_score(device: Device) -> float:
    """Heuristic score used to pick the richest representative among duplicates."""
    score = float(device.open_ports_count * 10)

    if device.fingerprint is not None:
        score += device.fingerprint.confidence * 100.0

    if device.hostname and device.hostname != "unknown":
        score += 15.0

    if device.mac_vendor and device.mac_vendor not in (
        "Unknown",
...
def merge_devices_by_mac(devices: list[Device]) -> list[Device]:
    """
    Collapse duplicate rows sharing the same MAC address.

    When a host appears more than once, this keeps one representative and merges
    richer signals (ports, fingerprint, hostname, vendor, latency, timestamps).
    """
    merged_by_mac: dict[str, Device] = {}

    for device in devices:
        existing = merged_by_mac.get(device.mac)
        if existing is None:
            merged_by_mac[device.mac] = device.model_copy(deep=True)
            continue

        existing_score = _device_quality_score(existing)
        candidate_score = _device_quality_score(device)

        # Keep the IP of the most informative record when duplicates disagree.
        if candidate_score > existing_score:
            existing.ip = device.ip

        # Merge operational flags and chronology.
        existing.is_online = existing.is_online or device.is_online
        if device.first_seen < existing.first_seen:
            existing.first_seen = device.first_seen
        if device.last_seen > existing.last_seen:
            existing.last_seen = device.last_seen

        # Prefer known hostname/vendor over placeholders.
        if existing.hostname == "unknown" and device.hostname != "unknown":
            existing.hostname = device.hostname
        if existing.mac_vendor in ("Unknown", "Locally Administered (Randomized)"):
            if device.mac_vendor not in (
                "Unknown",
                "Locally Administered (Randomized)",
            ):
                existing.mac_vendor = device.mac_vendor

        # Merge latency by taking the best available RTT (lowest).
        if existing.latency_ms is None:
            existing.latency_ms = device.latency_ms
        elif device.latency_ms is not None:
            existing.latency_ms = min(existing.latency_ms, device.latency_ms)

        # Merge ports without duplicates.
        for port in device.ports:
            existing.add_or_update_port(port)

        # Merge fingerprint evidence.
        if existing.fingerprint is None:
            if device.fingerprint is not None:
                existing.fingerprint = device.fingerprint.model_copy(deep=True)
        elif device.fingerprint is not None:
            existing.fingerprint = existing.fingerprint.merge(device.fingerprint)

    return list(merged_by_mac.values())
...
class Alert(BaseModel):
    """A SIEM alert generated when the scanner observes a suspicious signal."""

    alert_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()),
        description="UUID v4 — stable identifier across SSE / dashboard events.",
    )
    scan_id: Optional[str] = Field(None, description="Owning scan, if any.")
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="When the alert was raised.",
    )
    severity: AlertSeverity = Field(
        AlertSeverity.LOW, description="Severity level: low | medium | high | critical."
    )
    category: str = Field(
        ...,
        max_length=64,
        description="Short machine-readable kind, e.g. 'risky_port', 'new_host'.",
        examples=["risky_port", "host_change", "service_risk"],
    )
    title: str = Field(..., max_length=160, description="Single-line human title.")
    description: str = Field("", max_length=2048, description="Detailed explanation.")
    ip: Optional[str] = Field(None, description="Target IP, if applicable.")
    mac: Optional[str] = Field(None, description="Target MAC, if applicable.")
    port: Optional[int] = Field(None, ge=1, le=65535)
    service: Optional[str] = Field(None, max_length=64)
    resolved: bool = Field(False, description="True once an analyst acknowledges it.")
...
class ScanResult(BaseModel):
    ...
    duration_seconds: float = Field(
        0.0,
        ge=0.0,
        description="Total wall-clock time taken by this scan, in seconds.",
    )
    alerts: list[Alert] = Field(
        default_factory=list,
        description="SIEM alerts raised during this scan.",
    )
    ...
    def validate_network(cls, v: str) -> str:
        """Valide que le réseau est un CIDR IPv4 valide."""
        if "/" not in v:
            raise ValueError("Format CIDR requis (ex: /24)")
        try:
            # On utilise ip_network avec strict=False pour autoriser 192.168.1.1/24
            ipaddress.ip_network(v.strip(), strict=False)
            return v.strip()
        except Exception as e:
            raise ValueError(f"Réseau invalide : {v}. Erreur : {e}")
...
# Ports whose mere exposure warrants the corresponding risk level when no
# richer alert signal is available. High = remote-admin / legacy / DB / RCE
# surfaces; medium = common but routinely-attacked services.
_HIGH_RISK_PORTS: frozenset[int] = frozenset(
    {21, 23, 135, 139, 445, 1433, 1521, 2375, 3389, 5900, 6379, 9200, 27017}
)
_MEDIUM_RISK_PORTS: frozenset[int] = frozenset({22, 80, 3306, 8000, 8080, 8443})

# Alert severity → device risk bucket. Risk only has three buckets, so both
# critical and high alerts surface as "high".
_SEVERITY_TO_RISK: dict[AlertSeverity, str] = {
...
# Ordering used to pick the worst severity / risk among several.
_RISK_RANK: dict[str, int] = {"low": 1, "medium": 2, "high": 3}


def risk_from_ports(device: Device) -> str:
    """Port-exposure heuristic — the fallback when no alerts apply."""
    open_set = {p.number for p in device.get_open_ports()}
    if open_set & _HIGH_RISK_PORTS:
        return "high"
    if open_set & _MEDIUM_RISK_PORTS:
        return "medium"
    return "low"


def _alert_targets_device(alert: "Alert", device: Device) -> bool:
    """True when an alert is attributed to this device (by IP or MAC)."""
    if alert.ip and alert.ip == device.ip:
        return True
    if alert.mac and device.mac:
        try:
            return _normalize_mac(alert.mac) == device.mac
        except (ValueError, TypeError):
            return alert.mac == device.mac
    return False


def compute_device_risk(
    device: Device, alerts: Optional[list["Alert"]] = None
) -> str:
    """
    Compute a device's risk label (``"low"`` | ``"medium"`` | ``"high"``).

    Precedence:
      1. The highest severity among alerts linked to this device (by IP/MAC),
         mapped through :data:`_SEVERITY_TO_RISK`.
      2. Otherwise, the port-exposure heuristic (:func:`risk_from_ports`).
    """
    best: Optional[str] = None
    if alerts:
        for alert in alerts:
            if not _alert_targets_device(alert, device):
                continue
            risk = _SEVERITY_TO_RISK.get(alert.severity, "low")
            if best is None or _RISK_RANK[risk] > _RISK_RANK[best]:
                best = risk
    if best is not None:
        return best
    return risk_from_ports(device)
